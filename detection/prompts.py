...
# ─────────────────────────────────────────────
# 轻量角色识别 Prompt（每 chunk 调用，只判断 agent_speaker，不做合规检测）
# ─────────────────────────────────────────────

ROLE_DETECT_SYSTEM = """あなたは銀行営業会話の話者役割識別の専門家です。
音声認識（話者分離あり）で取得した会話断片を分析し、どの Speaker ID が営業員かを判断してください。

営業員の特徴：商品を説明している、顧客に購入を勧めている、挨拶や自己紹介をしている
顧客の特徴：質問をしている、不安を表明している、検討している、名前を呼ばれている

出力形式（厳密な JSON のみ、説明不要）：
{"agent_speaker": "0"}
"""

ROLE_DETECT_USER_TEMPLATE = """以下の会話断片の Speaker ID を分析し、営業員の Speaker ID を返してください。
